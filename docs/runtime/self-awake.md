...
- 按优先级执行；遇到首个可行动项即停止；只委派 1 个任务。
- 若无可行动项，必须回复："Self-check: no action needed." 然后休眠。
- Legend：C=Check / T=Trigger / D=Delegate / S=Skip；无历史=从未执行。
- 委派 prompt 前缀："[P#] "（例："[P2] Fix lint errors"）。
