...
| 状态 | 排名 | 自研能力（范围 LOC） | 建议第三库 | 净减行估算 | 功能收益 | 迁移成本 | ROI 指数 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 待办 | 1 | 文件持久化与并发串行（`786`） | `better-sqlite3` + `kysely` | `-350 ~ -490`（中位 `-420`） | `5` | `4` | `525` |
| 完成 | 2 | Prompt 模板 include/if 渲染器（`287`） | `nunjucks` | `-120 ~ -180`（中位 `-150`） | `4` | `2` | `300` |
| 完成 | 3 | Action 协议解析（`<M:...>` 标签提取，`374`） | `remark-parse` + `unist-util-visit`（HTML 节点遍历） | `-130 ~ -190`（中位 `-160`） | `4` | `3` | `213` |
| 待办 | 4 | 历史检索引擎 glue（`204`） | `@orama/orama`（或 `minisearch`） | `-80 ~ -140`（中位 `-110`） | `3` | `2` | `165` |
| 完成 | 5 | OpenAI Chat 流式 SSE 解析（`273`） | `openai` 官方 SDK（stream helper） | `-70 ~ -110`（中位 `-90`） | `4` | `3` | `120` |
| 完成 | 6 | SSE/ETag 手写协议层（`282`） | `fastify-sse-v2` + `@fastify/etag` | `-50 ~ -90`（中位 `-70`） | `3` | `2` | `105` |
...
- 完成记录（2026-02-26）：
  - 已切换到 `nunjucks`，模板语法统一为 `{{ }}` + `{% if %}`。
  - 已移除自研 `if/else/endif` 与 include 展开实现，不保留兼容层。
...
- 功能收益：
  ...
  - 标签嵌套与尾部碎片处理更稳
- 完成记录（2026-02-26）：
  - 已改为 `remark-parse` + `unist-util-visit` 遍历 HTML 节点提取 `<M:...>`。
  - 已移除 `htmlparser2/micromark` 对应实现，不保留兼容链路。
...
- 功能收益：
  ...
  - 错误类型与重试语义可复用 SDK
- 完成记录（2026-02-26）：
  - 已切换为 `openai` SDK 的 `chat.completions.stream` helper。
  - 已移除手写 SSE 读流与 chunk 解析逻辑。
...
- 完成记录（2026-02-26）：
  - SSE 已切换到 `fastify-sse-v2` 插件。
  - ETag 已切换到 `@fastify/etag` 插件并删除手写比对逻辑。
...
1. `#1 文件持久化 -> SQLite`（最大总收益）
2. `#4 历史检索 -> Orama/MiniSearch`
