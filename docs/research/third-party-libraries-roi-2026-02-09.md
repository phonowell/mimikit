...
| 5 | `better-sqlite3`（轻 DAO） | JSONL 队列与游标状态 | ~302 | 原子性/游标查询/裁剪一致性更强，后续统计查询更容易 | 中高：涉及持久化格式迁移 | 6.9 |
...
- `p-queue`/`p-retry` 候选：`src/worker/dispatch.ts:61` `src/worker/run-retry.ts:78` `src/manager/action-apply.ts:78`
...
- `better-sqlite3` 候选：`src/storage/jsonl.ts:1` `src/streams/queues.ts:1` `src/storage/task-progress.ts:23`
...
- 行为变化：
  ...
  - `runtime-state.json`：仅接受新 queues 结构，不再兼容历史平铺 cursor 字段。
...
| action | 形态 | 三方可替代 | 结论 |
|---|---|---|---|
| `create_task` / `cancel_task` / `summarize_task_result` | 编排逻辑 | 无直接通用库 | 维持自研 |
| （已移除） | 协议动作 | - | 已清理 |
