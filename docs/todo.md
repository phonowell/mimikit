...
目标：让主会话在异步任务完成后可获得新上下文，并即时继续决策。

- 提交任务时建立任务与主会话的关联关系。
- 任务完成后写入结果并实时唤醒主会话。
- 主会话只消费 `inputs/results` 增量数据。
...
- 采用事件驱动唤醒，不采用固定轮询。
- 统一唤醒入口，仅保留三类实时触发：`user_input`、`task_result`、`cron`。
- 主会话执行采用单飞策略，同一时刻仅允许一个活跃运行。
- 若主会话正在运行，新事件进入待处理队列，避免并发重入。
- 不保留额外唤醒事件队列，唤醒通过进程内 signal 实时触发。

Cron 的默认策略：

1. Cron 先触发本地执行层动作（创建任务、进入队列）。
2. 只要本轮有 Cron 触发，立即唤醒主会话。
3. 若主会话忙碌，保持单飞，不并发重入。
