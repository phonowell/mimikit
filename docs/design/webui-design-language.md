...
- 三类消息气泡统一保留极细边框（`1px` 低对比边框）保证边界可读性。
- `user`：使用 `--bubble-user-bg`，用于轻强调。
- `agent`：保持中性面板色。
- `system`：使用 `--bubble-system-bg` 区分系统提示。
- 颜色语义：消息区背景为浅灰；`agent` 近白、`user` 浅蓝、`system` 略深于背景的中灰；并为三类气泡配置独立字体色。
...
- 所有按钮文本不可选中/复制（`user-select: none`）。
...
- 视觉微调（二十二）：气泡语义再校准为“背景灰 + agent 近白 + user 浅蓝 + system 较深灰”。
- 视觉微调（二十三）：强化 `agent` 气泡边框可见度，同时弱化 `system` 气泡边框。
- 视觉微调（二十四）：系统气泡整体弱化，背景更贴近消息区灰底，边框与文字对比进一步下调。
