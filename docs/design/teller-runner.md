...
4. 若调用失败或无可用摘要，走兜底：
   - 最新用户输入
   - 或固定文案
5. 返回 `TellerDigest`：`digestId`、`summary`、`inputs`、`results`（当前固定空数组）、`taskSummary`。
...
2. 调用 `runApiRunner`。
3. 若输出非空，直接返回该文本。
4. 若异常或空文本，走 fallback：`decision` 原文 → `inputIds` 对应输入 → 固定“收到（timestamp）”。
