...
provider 层内聚到 `src/providers`，形成可迁移最小闭环：
...
1. 将 provider 对 `shared` 的直接依赖收敛到 provider 内部模块：
   - thread id 能力迁入 `src/providers/thread-id.ts`
   - usage normalize 与 id/stripUndefined 能力迁入 `src/providers/utils.ts`
2. 将 provider 对 `types` 的耦合收敛：
   - provider 层 token usage 类型独立为 `src/providers/token-usage.ts`
3. 将 provider 配置读取与日志写入所需的最小能力收敛到 provider 内：
   - `src/providers/fs.ts`
   - `src/providers/log.ts`
   - `src/providers/safe.ts`
4. 更新调用侧引用：
   - `worker` / `manager` 的 provider-thread-id 读取统一改为 `src/providers/thread-id.ts`
