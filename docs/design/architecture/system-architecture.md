...
- manager prompt 收敛为双 packet：`state_packet` 负责稳定状态（focus/task/plan），其中 task 与 plan 只暴露合同级 digest（`goal/scope/acceptance` 等）与调度外壳，不回灌完整 worker prompt；`event_packet` 负责当前批次事件（input/result/history/environment/packet）；详细 task result 只留在 `event_packet.batch_results`，`state_packet.tasks` 不再重复展开结果正文；section 字节预算固定取自 `manager.promptSections`，`wakeProfile` 只影响 packet section 选择，不再动态改写 bytes，也不再分档 action surface。
- manager 的 action 授权只保留三层：结构契约、runtime legality、风险门禁。默认续跑判断只依赖 runtime state、对象归属与当前风险边界；高风险动作必须由当前用户输入直接授权，低风险 continuation 不再要求额外 continuation 协议位、语义自证或 validation 预决策。
...
- 当 action 校验或执行反馈不足以继续推进时，manager 当轮直接降级为澄清答复，不再进入二次自修回合或掉进 `manager_end status=error`。
