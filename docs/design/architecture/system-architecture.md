...
- manager 的默认续跑判断只把结构化锚点与运行态关系（如 `continuation_of`、`plan.runtime.lastTaskId`、当前 result task 对齐）当作 provenance 与候选过滤信号；真正决定是否继续的是语义一致、对象归属一致与 runtime provenance 的组合，而不是任一单独弱信号。
