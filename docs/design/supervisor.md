...
`triggerCheckMs` 控制 conditional 评估间隔，`triggerStuckMs` 用于清理卡死触发器运行状态。当只有用户输入且无用户可见事件时，按 `teller.debounceMs` 收集连续输入；若用户空窗超过 `teller.returnAfterMs`，首条输入不做 debounce。
## 内部结果判定（确定性）
...
Planner 和 Worker 各自有独立的队列、运行目录、结果目录，互不干扰。并发上限由配置项 `concurrency` 控制。
## 恢复机制
