...
export const bindFocusInteractions = (focusesList) => {
  ...
  const onListClick = (event) => {
    ...
    const link = target.closest('.focus-link')
    if (!link) return
    ...
    if (!opened) console.warn('[webui] open focus archive failed', 'popup blocked')
  }

  focusesList.addEventListener('click', onListClick)

  return () => {
    focusesList.removeEventListener('click', onListClick)
  }
}
