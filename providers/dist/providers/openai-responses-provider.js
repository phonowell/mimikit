...
import { buildProviderAbortedError, buildProviderPreflightError, buildProviderSdkError, buildProviderTimeoutError, isTransientProviderMessage, ProviderError, readProviderErrorCode, } from './provider-error.js';
...
const trimNonEmptyString = (value) => {
    const trimmed = value?.trim();
    return trimmed && trimmed.length > 0 ? trimmed : undefined;
};
const resolveApiKey = (apiKey, requiresAuth) => {
...
const resolveModel = (request, fallbackModel) => {
    const requestModel = request.model?.trim();
    const fallback = fallbackModel?.trim();
    const model = (requestModel && requestModel.length > 0 ? requestModel : undefined) ??
...
const appendOpenAiResponsesLog = async (request, entry) => {
    if (!request.logPath)
        return;
    await bestEffort('appendLog: llm_call', () => appendLog(request.logPath, {
        ...entry,
...
const readCompletedOutput = (completed) => {
    ...
    for (const item of output) {
        ...
        if (asString(itemRecord, 'type') !== 'message')
            continue;
        ...
        for (const part of content) {
            ...
            if (asString(partRecord, 'type') !== 'output_text')
                continue;
...
const resolveSessionId = (threadId) => {
    const current = threadId?.trim();
    if (current)
        return current;
    return `session-${randomUUID()}`;
};
const toPromptSegments = (request) => {
...
const buildResponsesInput = (request) => {
...
export const parseResponsesSse = (raw) => {
    ...
    if (!completed)
        throw new Error('responses_completed_event_missing');
...
export const parseResponsesJson = (raw) => {
    const payload = parseJsonRecord(raw);
    if (!payload)
        throw new Error('responses_json_parse_failed');
...
export const parseResponsesPayload = (raw) => {
    const trimmed = raw.trimStart();
    if (trimmed.startsWith('{') || trimmed.startsWith('['))
        return parseResponsesJson(raw);
    return parseResponsesSse(raw);
};
const buildHttpErrorMessage = (status, raw) => {
    const message = readErrorMessage(raw);
    if (message)
        return `responses_http_${status}:${message}`;
...
const runOpenAiResponses = async (request) => {
    ...
    try {
        ...
        const apiKey = resolveApiKey(requestApiKey ?? settings.apiKey, settings.requiresAuth);
        const dispatcher = resolveProxyDispatcher(request.proxy);
        const model = resolveModel(request, settings.model);
        const endpoint = `${baseUrl}/responses`;
        const shouldStripAuthorizationHeader = settings.requiresAuth === false &&
            !requestApiKey &&
        ...
        if (err instanceof ProviderError)
            ...
            const transient = /responses_http_(429|5\d\d):/i.test(err.message) ||
