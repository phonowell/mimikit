...
- manager 默认模型原则：若无明确 ROI/成本理由，manager 默认模型与推理强度不得长期低于主 worker 档位；禁止让“主脑默认更弱”成为产品常态。
...
- 新用例准入：任何新增测试用例在落地前都必须先做一次 ROI 评测；只有当它覆盖真实风险/回归点、且不依赖词表驱动断言或关键词列表判定时，才允许进入仓库
...
- 测试词表驱动禁令：测试不得通过关键词列表、同义词枚举、标签文案清单或固定措辞命中来证明核心行为成立；若无法脱离这类词表驱动检查，该用例默认视为低 ROI，不应落地
