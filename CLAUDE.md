...
- `plan` 仍偏自由文本任务容器，触发后还要二次解释，偏离“编排层只调度结构化 effect”的目标。
- `focus` 仍可能滑向任务板：`summary/openItems` 是 digest，不应承载执行步骤、验收标准、恢复指令。
- `memory` 刷新链路仍可能吸收过程态；长期记忆应只保留稳定事实、偏好、约束，不应承载当前任务进度或待办。
- `worktree` 物化已存在，但 `review -> merge -> cleanup` 还没成为最小执行协议；编排链路仍缺闭环状态。
- idle 生命周期虽已存在，但“哪些后台任务只允许维护派生物、哪些绝不能改主状态”尚未写成硬约束。
- 主线程低上下文、subagent 默认处理局部任务这件事仍未固化为长期操作规则，容易在实现和编排时回到主线程堆上下文。
