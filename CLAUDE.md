...
- 定位：产品是异步自治作业系统；实现是单 session、承担推进责任的编排中层，负责意图理解、任务编排、日常判断、状态治理与验收门禁，不直接执行任务。
- 特点：适配长时间异步窗口，内置 WebUI；执行链路委托外部运行时；manager 在用户离线时默认继续推进并做常规判断，只在高风险、证据不足或目标冲突时上提；用户返回后仍可复盘、确认与续跑。
...
- 面向无人在线时段的异步自治作业系统。
- 基于 codex 开箱能力构建的单 session 编排中层。
- 负责意图理解、任务编排、日常推进判断、状态治理、验收门禁与例外上提，不直接承担主要执行。
- 默认在用户离线时继续推进，目标是显著降低用户跟进成本。
...
- 把主 agent 收敛为极简、特化、承担推进责任的编排中层，只保留目标、计划、当前状态与验收门禁。
- 默认把搜索、实现、排查、细读等局部工作委派给 subagent 或外部运行时，并只接收压缩后的结论与工件。
- 让 manager 在用户离线时基于当前 `focus`、任务合同、结果与门禁默认继续推进，并做常规判断；只有在高风险、证据不足、目标冲突或多次纠偏失败时才上提。
...
- 不把 worker 的原始建议或大段结果原样甩给用户，逼用户自己承担中层跟进职责。
