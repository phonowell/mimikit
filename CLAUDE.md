...
- 空闲生命周期只做派生维护任务：允许在 idle 窗口执行记忆刷新、归档整理、历史压缩、低优先级总结；禁止后台任务直接改写任务真相源、调度主状态或用户显式决策。
...
- 以文件系统为主状态层，以 WebUI 作为复盘、确认与续跑入口。
...
- idle 维护约束：记忆压缩、问题总结、归档整理等后台任务只允许产出派生工件，不得静默改写 `task/plan/focus` 真相源。
