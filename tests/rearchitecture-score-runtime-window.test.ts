...
test('scoreRuntimeWindow computes core governance metrics without not_collected blockers', async () => {
  ...
  await Promise.all([
    ...
    writeFile(
      join(stateDir, 'history', '2026-03-08.jsonl'),
      `${[
        JSON.stringify({
          id: 'hist-1',
          role: 'user',
    ...
    writeFile(
      join(stateDir, 'task-progress', '2026-03-08', 'task-1.jsonl'),
      `${[
        JSON.stringify({
          taskId: 'task-1',
          type: 'worker_start',
  ...
  await appendJsonl(join(stateDir, 'results', 'packets.jsonl'), [
    {
      ...
      payload: {
        ...
        evidence: {
          ...
          acceptanceChecks: [{ criterion: 'has result', met: true }],
          stateDelta: {
