...
test('set_plan(write) update stays blocked when the user only describes the new lane with natural-language aliases', () => {
  const currentPlan = createPlanFixture({
    id: 'plan-auth-guard-lane-shift-alias-only',
  ...
  const feedback = collectManagerActionFeedback(
    ...
    ],
    {
      inputs: [
        createUserInput(
          `把 ${currentPlan.id} 这个计划改掉，切到 /repo/billing，并且用 worktree、写入模式继续推进。`,
        ),
      ],
