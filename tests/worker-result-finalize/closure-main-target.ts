...
test('finalizeResult binds closure task to main even when repo root is checked out elsewhere', async () => {
  ...
  execFileSync('git', ['checkout', '-b', 'ops/repo-root-active'], {
    cwd: repoRoot,
    stdio: 'ignore',
  })
  await writeFile(
  ...
  const task: Task = {
    ...
    fingerprint: 'task-source-closure-main-target',
    semanticKey: 'task-source-closure-main-target',
