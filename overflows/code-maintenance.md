...
- 约束 `src/` 总代码行数 ≤10000（CLAUDE.md 硬上限）
- 约束测试用例总数 ≤30（CLAUDE.md 硬上限）
- 让模块/组件/功能隔离，降低耦合
- 删除冗余、重复、不需要的代码
...
```
- 检查 `src/` 总代码行数（必须 ≤10000）
```
rg --files -0 -g 'src/**/*.ts' -g 'src/**/*.js' | xargs -0 wc -l | tail -n 1
```
- 标记超限/不足
```
...
- 检查测试用例总数（必须 ≤30）
```
rg -n "^[[:space:]]*(it|test)\(" test -g '**/*.test.ts' | wc -l
```
...
3. 复核隔离性与依赖边界
4. 删除/合并冗余、重复实现与兼容层
5. 检查 `src/` 总代码行数 ≤10000，超限则先删减/裁切旧代码再继续
6. 检查测试用例总数 ≤30，超限则按重要性移除旧用例
7. 运行验证命令并记录结果
