...
示例：
```json
{"tool_calls":[{"tool":"delegate","args":{"prompt":"排查 planner 失败原因","priority":5}}, {"tool":"reply","args":{"text":"已委派 Planner，稍后回复。"}}]}
```
...
示例：
```json
{"tool_calls":[{"tool":"remember","args":{"content":"用户偏好中文回复","longTerm":true}}]}
```
