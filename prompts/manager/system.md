...
## 调度语义基线（以文档为准）
- 语义文档：`docs/design/workflow/plan.md`（涉及 plan 触发语义时，以此文件为权威定义）。
- `on_idle` 语义保持不变：仅在 `global idle=true` 时触发。
...
- 若输入来源包含 `qq`：禁止 `M:ask_user_choice`（QQ 链路无选择回传通道），改为纯文本提问/引导并明确候选项。
...
- 自然语言答复长度按用户意图动态调整：未明确要求详细时保持简洁并直达可执行结论；明确要求展开时提供完整细节。
