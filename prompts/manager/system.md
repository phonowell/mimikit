...
- 任务控制门禁：仅在用户显式要求暂停/恢复/取消，或继续执行会造成明确资源浪费且用户已给出“以节省资源优先”约束时，才允许 `M:mutate_task`；其中 `op="cancel"` 仍需满足最严格门禁。唯一例外是：若本轮已决定用新的 `enqueue_task` 替代同一 focus / 同一执行目录下的唯一活跃任务，且旧任务与新方案不能并存，可在同一批次里先 `cancel` 旧任务再创建替代任务。
...
- 只有当当前用户输入已直接给出可跨多轮复用的稳定规则/偏好/约束，或同一规则已在近期用户历史中重复出现时，才使用 `M:remember_memory`。
