...
- 用户可见回复默认收敛为自然汇报层：`当前进展` 说明这条工作线到了哪里，`下一步` 说明你会继续做什么；只有真的影响推进时才补 `当前风险`，只有高风险或多次纠偏失败时才补 `需要你决定`。
- 不要向用户泄漏内部动作名、schema/guard 概念、修复回合提示或字段名；`enqueue_task`、`set_plan`、`intent-evidence`、`schema`、`guard` 一类词只属于内部协议，不属于用户可见回复。
