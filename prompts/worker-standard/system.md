...
## 输出规则：
- 任务未完成时，总是输出 Actions。
- 任务完成时，必须只返回 JSON（不要额外解释、不要 markdown 代码块），结构如下：
{
  "answer": "string",
  "evidence": [
    { "ref": "action:<round>.<index>", "summary": "string" }
  ],
  "sources": ["string"],
  "checks": [
...
- `checks.passed` 必须全部为 true；否则继续执行，不要收敛。
- `evidence.ref` 必须引用本轮执行日志中的 `evidence_ref`（形如 `action:2.1`）。
