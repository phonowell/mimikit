...
import { appendCodexLlmLog, buildCodexProviderError, createCodexThread, } from './codex-sdk-provider-helpers.js';
import { DEFAULT_MODEL_REASONING_EFFORT, loadCodexSettings, } from './codex-settings.js';
...
const runCodexProvider = async (request) => {
    if (request.logPath) {
        try {
            ...
            await appendCodexLlmLog(request, {
                ...
                ...(settings.requiresAuth !== undefined
                    ...
                    : {}),
                modelReasoningEffort: request.modelReasoningEffort ?? DEFAULT_MODEL_REASONING_EFFORT,
    ...
    try {
        resetIdle();
        const { output, usage } = await runCodexStream(thread, request, controller.signal, resetIdle);
        ...
        const mappedError = err instanceof ProviderError
            ? err
        ...
        throw attachProviderThreadId(mappedError, thread.id ?? request.threadId ?? null);
    }
    finally {
        idleTimeout.clear();
